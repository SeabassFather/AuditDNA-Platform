...
export default function ServiceRequestCard({ serviceName }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert(`Ã¢Å“â€¦ Submitted! Reference: ${result.caseId || "ok"}`);
    } catch (err) {
      console.error("Upload failed", err);
      alert("Ã¢ÂÅ’ Failed to submit request.");
      alert(`ÃƒÆ’Ã‚Â¢Ãƒâ€¦Ã¢â‚¬Å“ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦ Submitted! Reference: ${result.caseId || "ok"}`);
    } catch (err) {
      console.error("Upload failed", err);
      alert("ÃƒÆ’Ã‚Â¢Ãƒâ€šÃ‚ÂÃƒâ€¦Ã¢â‚¬â„¢ Failed to submit request.");
    }
  };
