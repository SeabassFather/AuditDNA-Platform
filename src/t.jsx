...
export default function EnvironmentalAnalyzer({ locationData, onAnalysisComplete }) {
  const [envData] = useState({
    ...
    moonPhase: 'Full Moon ÃƒÂ°Ã…Â¸Ã…â€™Ã¢â‚¬Â¢', currentTemp: 72, maxTemp: 85, minTemp: 55,
    humidity: 65, windSpeed: 8, pressure: 30.12
  });

  return (
    <div style={{background: '#1e293b', padding: '30px', borderRadius: '16px', border: '2px solid #f59e0b'}}>
      <h2 style={{color: '#f59e0b', marginBottom: '20px'}}>ÃƒÂ°Ã…Â¸Ã…â€™Ã…Â¾ Environmental Data</h2>
      ...
      <button onClick={() => onAnalysisComplete(envData)} style={{width: '100%', padding: '16px', background: 'linear-gradient(135deg, #f59e0b, #d97706)', border: 'none', borderRadius: '12px', color: 'white', fontSize: '1.2rem', fontWeight: '700', cursor: 'pointer'}}>ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Save Environmental Data</button>
    </div>
  );
}
