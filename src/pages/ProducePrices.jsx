...
export default function ProducePrices() {
  ...
  const options = useMemo(
    () => (!q ? ALL : fuse.search(q).map((r) => r.item.name)),
    [q],
  );
  const dataFor = prices[commodity] || {};
  const series = years
    .filter((y) => Array.isArray(dataFor[y]))
    .map((y) => ({ name: y, data: (dataFor[y] || []).slice(1, 27) }));
  const average = useMemo(() => {
    const out = Array(26).fill(null);
    for (let i = 0; i < 26; i++) {
      const vals = series
        .map((s) => s.data[i])
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold flex items-center gap-2">
          <Leaf className="w-6 h-6" /> USDA Produce Pricing
        </h1>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="md:col-span-1 bg-white rounded-2xl shadow p-4">
          <div className="flex items-center gap-2 mb-3 border rounded-xl px-3 py-2">
            ...
            {options.map((name) => (
              <button
                ...
                className={
                  ...
                  (commodity === name ? "bg-green-100" : "hover:bg-gray-100")
                }
              >
                {name}
              </button>
        ...
        <div className="md:col-span-2">
          <div className="mb-2 text-sm text-gray-500">
            Commodity: <span className="font-medium">{commodity}</span> View:
