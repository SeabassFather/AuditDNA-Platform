...
async function httpGetOne(name) {
  const r = await fetch(`/api/commodity/${encodeURIComponent(name)}`);
  if (!r.ok) throw new Error(`HTTP ${r.status}`);
  return r.json();
}

export async function getCommodities() {
  if (MODE === "http") return httpList();
  const db = loadBundle();
  return Object.keys(db).sort();
}
export async function getCommodityData(name) {
  if (MODE === "http") return httpGetOne(name);
  const db = loadBundle();
  return db[name];
}
...
export function normalizeCommodityRecord(raw) {
  ...
  for (const y of Object.keys(ys)) {
    ...
    out.years[y] = arr.map((v) =>
      Number.isFinite(v) ? Number(v) : v == null ? null : Number(v),
    );
  }
  return out;
}
