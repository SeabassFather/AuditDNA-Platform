...
async function parsePdf(filePath) {
  ...
  const results = [];
  const re = /([A-Za-z0-9\.\s\-\(\)\/]+)\s+([0-9\.,]+)\s*(mg\/kg|ppm|ppb|cfu\/100g|cfu\/g|Âµg\/kg|ug\/kg|ng\/g)?/i;
...
Write-Host "  â€¢ add sample parsing unit tests" -ForegroundColor White
Write-Host "  â€¢ generate curl examples to test Stripe & uploads" -ForegroundColor White
Write-Host "  â€¢ create a single-run installer that also runs npm install automatically (requires user confirmation)" -ForegroundColor White
Write-Host "======================================================" -ForegroundColor Cyan
'@
