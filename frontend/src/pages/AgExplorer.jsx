...
export default function AgExplorer() {
  ...
  // Endpoints via env or config (leave blank = not configured)
  ...
  // PRODUCE SEARCH: UI only, no fake results (shows hints)
  const hints = useMemo(() => {
  ...
  // FACTORING (UI + math only, no mock data)
  const [invoice, setInvoice] = useState(0);
  const [rate, setRate] = useState(0); // % per 30d
  const [invoice, setInvoice] = useState(0);
          ...
          <div className="card">
            ...
            <div className="kv">
              ...
              <input
                ...
                type="number"
                value={acre}
                onChange={(e) => setAcre(+e.target.value || 0)}
              />
            </div>
            <div className="kv">
              <span>Flow (GPM)</span>
              <input
                ...
                type="number"
                value={gpm}
                onChange={(e) => setGpm(+e.target.value || 0)}
              />
            </div>
            <div className="kv">
              <span>Hours/day</span>
            ...
            <div className="subtext">
              {gallonsPerDay
                ? gallonsPerDay.toLocaleString() + " gal/day"
                : "enter values"}
            </div>
            <div className="subtext">
              {gallonsPerAcre
                ? gallonsPerAcre.toLocaleString() + " gal/acre/day"
                : ""}
            </div>
          </div>
          <div className="card">
            ...
            <div className="empty">
              add emitter spacing, ET, schedule when backend is ready
            </div>
          </div>
        </div>
      )}
          ...
          <div className="card">
            ...
            <div className="kv">
              ...
              <input
                ...
                type="number"
                value={invoice}
                onChange={(e) => setInvoice(+e.target.value || 0)}
              />
            </div>
            <div className="kv">
              <span>Rate % / 30d</span>
            ...
            <div className="subtext">
              Advance 85% (static policyadjust once backend provides product
              terms)
            </div>
            <div className="subtext">
              Advance: {advance ? "$" + advance.toLocaleString() : ""} | Fee:{" "}
              {fee ? "$" + fee.toLocaleString() : ""} | Net:{" "}
