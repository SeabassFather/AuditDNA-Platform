  ...
  return (
    <div style={{ padding: '32px' }}>
      ...
      <div style={{ display: 'flex', gap: '16px', overflowX: 'auto', paddingBottom: '10px' }}>
        {tickers.mortgage.length > 0 && <LiveTicker data={tickers.mortgage} title="Mortgage" icon="ðŸ " />}
        {tickers.commodities.length > 0 && <LiveTicker data={tickers.commodities} title="Commodities" icon="ðŸŒ¾" />}
        {tickers.markets.length > 0 && <LiveTicker data={tickers.markets} title="Markets" icon="ðŸ“ˆ" />}
        {tickers.mortgage.length > 0 && <LiveTicker data={tickers.mortgage} title="Mortgage" icon="ÃƒÂ°Ã…Â¸Ã‚ÂÃ‚Â " />}
        {tickers.commodities.length > 0 && <LiveTicker data={tickers.commodities} title="Commodities" icon="ÃƒÂ°Ã…Â¸Ã…â€™Ã‚Â¾" />}
        {tickers.markets.length > 0 && <LiveTicker data={tickers.markets} title="Markets" icon="ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã‹â€ " />}
      </div>
    </div>
  );
}
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '24px' }}>
          ...
          <button onClick={onClose} style={{ background: 'none', border: 'none', fontSize: '24px', cursor: 'pointer' }}>ÃƒÆ’Ã¢â‚¬â€</button>
        </div>

        {success ? (
          <div style={{ textAlign: 'center', padding: '40px', background: '#f0fdf4', borderRadius: '12px' }}>
            <div style={{ fontSize: '48px', marginBottom: '16px' }}>âœ…</div>
            <div style={{ fontSize: '48px', marginBottom: '16px' }}>ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦</div>
...
function ServicesTab() {
  ...
  const categories = [
    {
      name: 'USDA Pricing & Compliance', icon: 'ðŸŒ¾', color: '#10b981',
      name: 'USDA Pricing & Compliance', icon: 'ÃƒÂ°Ã…Â¸Ã…â€™Ã‚Â¾', color: '#10b981',
    ...
    {
      name: 'WaterTech & Soil Analysis', icon: 'ðŸ’§', color: '#06b6d4',
      name: 'WaterTech & Soil Analysis', icon: 'ÃƒÂ°Ã…Â¸Ã¢â‚¬â„¢Ã‚Â§', color: '#06b6d4',
    ...
    {
      name: 'Mortgage & Real Estate', icon: 'ðŸ ', color: '#667eea',
      name: 'Mortgage & Real Estate', icon: 'ÃƒÂ°Ã…Â¸Ã‚ÂÃ‚Â ', color: '#667eea',
    ...
    {
      name: 'Factoring & PO Finance', icon: 'ðŸ’°', color: '#f59e0b',
      name: 'Factoring & PO Finance', icon: 'ÃƒÂ°Ã…Â¸Ã¢â‚¬â„¢Ã‚Â°', color: '#f59e0b',
    ...
    {
      name: 'Insurance & Risk', icon: 'ðŸ›¡ï¸', color: '#14b8a6',
      name: 'Insurance & Risk', icon: 'ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂºÃ‚Â¡ÃƒÂ¯Ã‚Â¸Ã‚Â', color: '#14b8a6',
    ...
    {
      name: 'Global Compliance', icon: 'âš–ï¸', color: '#8b5cf6',
      name: 'Global Compliance', icon: 'ÃƒÂ¢Ã…Â¡Ã¢â‚¬â€œÃƒÂ¯Ã‚Â¸Ã‚Â', color: '#8b5cf6',
      desc: 'GDPR, CCPA, PIPEDA, AML/KYC',
      services: ['GDPR Audit', 'CCPA Review', 'PIPEDA Check', 'AML/KYC', 'Contract Analysis', 'Risk Heatmap']
    },
    {
      name: 'Education & Records', icon: 'ðŸŽ“', color: '#6366f1',
    ...
    {
      name: 'Business', icon: 'ðŸ¢', color: '#f43f5e',
      services: ['Tax Audit', 'Corporate Review', 'Lease Audit', 'Franchise Analysis']
    },
    {
      name: 'Consumer Protection', icon: 'ðŸ›’', color: '#84cc16',
      name: 'Education & Records', icon: 'ÃƒÂ°Ã…Â¸Ã…Â½Ã¢â‚¬Å“', color: '#6366f1',
      services: ['FERPA Compliance', 'Transcript Validation', 'Scholarship Verification', 'Credit Transfer']
    },
    {
      name: 'Healthcare', icon: 'ÃƒÂ°Ã…Â¸Ã‚ÂÃ‚Â¥', color: '#ec4899',
      services: ['HIPAA Audit', 'Medical Billing', 'Insurance Claims', 'Provider Credentialing']
    },
    {
      name: 'Business', icon: 'ÃƒÂ°Ã…Â¸Ã‚ÂÃ‚Â¢', color: '#f43f5e',
      services: ['Tax Audit', 'Corporate Review', 'Lease Audit', 'Franchise Analysis']
    },
    {
      name: 'Consumer Protection', icon: 'ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂºÃ¢â‚¬â„¢', color: '#84cc16',
      ...
      {categories.map((cat, idx) => (
        <div key={idx} style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '12px', marginBottom: '12px', overflow: 'hidden' }}>
          <button onClick={() => setExpandedCategory(expandedCategory === idx ? null : idx)}
            ...
            <div style={{ fontSize: '20px', color: '#64748b', transform: expandedCategory === idx ? 'rotate(180deg)' : 'rotate(0)', transition: 'transform 0.3s' }}>â–¼</div>
            <div style={{ fontSize: '20px', color: '#64748b', transform: expandedCategory === idx ? 'rotate(180deg)' : 'rotate(0)', transition: 'transform 0.3s' }}>ÃƒÂ¢Ã¢â‚¬â€œÃ‚Â¼</div>
          </button>
...
function SearchEngines() {
  ...
  const engines = [
    { key: 'usda', label: 'USDA Pricing', labelEs: 'Precios USDA', icon: 'ðŸŒ¾' },
    { key: 'watertech', label: 'WaterTech', labelEs: 'Agua y Suelo', icon: 'ðŸ’§' },
    { key: 'mortgage', label: 'Mortgage', labelEs: 'Hipotecaria', icon: 'ðŸ ' },
    { key: 'factoring', label: 'Factoring', labelEs: 'Factoraje', icon: 'ðŸ’°' },
    { key: 'mexico', label: 'Mexico RE', labelEs: 'Bienes RaÃ­ces', icon: 'ðŸ‡²ðŸ‡½' },
    { key: 'compliance', label: 'Compliance', labelEs: 'Cumplimiento', icon: 'âš–ï¸' },
    { key: 'upload', label: 'Upload', labelEs: 'Subir', icon: 'ðŸ“¤' }
    { key: 'usda', label: 'USDA Pricing', labelEs: 'Precios USDA', icon: 'ÃƒÂ°Ã…Â¸Ã…â€™Ã‚Â¾' },
    { key: 'watertech', label: 'WaterTech', labelEs: 'Agua y Suelo', icon: 'ÃƒÂ°Ã…Â¸Ã¢â‚¬â„¢Ã‚Â§' },
    { key: 'mortgage', label: 'Mortgage', labelEs: 'Hipotecaria', icon: 'ÃƒÂ°Ã…Â¸Ã‚ÂÃ‚Â ' },
    { key: 'factoring', label: 'Factoring', labelEs: 'Factoraje', icon: 'ÃƒÂ°Ã…Â¸Ã¢â‚¬â„¢Ã‚Â°' },
    { key: 'mexico', label: 'Mexico RE', labelEs: 'Bienes RaÃƒÆ’Ã‚Â­ces', icon: 'ÃƒÂ°Ã…Â¸Ã¢â‚¬Â¡Ã‚Â²ÃƒÂ°Ã…Â¸Ã¢â‚¬Â¡Ã‚Â½' },
    { key: 'compliance', label: 'Compliance', labelEs: 'Cumplimiento', icon: 'ÃƒÂ¢Ã…Â¡Ã¢â‚¬â€œÃƒÂ¯Ã‚Â¸Ã‚Â' },
    { key: 'upload', label: 'Upload', labelEs: 'Subir', icon: 'ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã‚Â¤' }
  ];
...
function Sidebar({ sidebarOpen, activeTab, setActiveTab }) {
  const items = [
    ...
    { key: 'services', label: 'Services', icon: 'ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂºÃ‚Â¡ÃƒÂ¯Ã‚Â¸Ã‚Â' },
    { key: 'search', label: 'Search', icon: 'ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Â' }
  ];
    ...
    <div style={{ background: 'linear-gradient(135deg, #f0f4f8, #e1e8ed)', minHeight: '100vh' }}>
      ...
      <div style={{ marginLeft: sidebarOpen ? '280px' : '0', transition: 'margin-left 0.3s' }}>
        <button onClick={() => setSidebarOpen(!sidebarOpen)}
          ...
          {sidebarOpen ? 'â†' : 'â†’'}
          {sidebarOpen ? 'ÃƒÂ¢Ã¢â‚¬Â Ã‚Â' : 'ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢'}
        </button>
