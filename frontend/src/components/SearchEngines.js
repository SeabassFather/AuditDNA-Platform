...
export default function SearchEngines({ activeSearchEngine, setActiveSearchEngine }) {
  ...
  const engines = [
    ...
    { id: 'watertech', en: 'WaterTech', es: 'AnÃ¡lisis de Agua', icon: FileText, color: '#2196f3' },
    { id: 'mortgage', en: 'Mortgage', es: 'Hipotecas', icon: Home, color: '#ff9800' },
    { id: 'factoring', en: 'Ag Factoring', es: 'Factoraje AgrÃ­cola', icon: DollarSign, color: '#9c27b0' },
    { id: 'mexico', en: 'Mexico RE', es: 'Bienes RaÃ­ces MX', icon: Briefcase, color: '#f44336' },
      ...
      {activeSearchEngine === 'usda' && (
        <div style={s.card}>
          ...
          <button onClick={handleSearch} style={{ ...s.btn('#4caf50'), marginTop: '20px' }} disabled={loading}>
            {loading ? 'â³ Searching...' : <><SearchIcon size={20} />Search Prices</>}
