...
export default function ServiceRequestCard({ serviceName }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert(`âœ… Submitted! Reference: ${result.caseId || "ok"}`);
    } catch (err) {
      console.error("Upload failed", err);
      alert("âŒ Failed to submit request.");
      alert(`ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Submitted! Reference: ${result.caseId || "ok"}`);
    } catch (err) {
      console.error("Upload failed", err);
      alert("ÃƒÂ¢Ã‚ÂÃ…â€™ Failed to submit request.");
    }
  };
